...
seed = 1;
X_test = LorenzFn(xrange, yrange, zrange, round(.1*numICs), tSpan, seed);
...
seed = 2;
X_val = LorenzFn(xrange, yrange, zrange, round(.2*numICs), tSpan, seed);
...
for j = 1:6
	seed = 2+j;
	X_train = LorenzFn(xrange, yrange, zrange, round(.7*numICs), tSpan, seed);
