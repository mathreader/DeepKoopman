...
def define_loss(x, y, g_list, weights, biases, params):
    ...
    loss_comparison_30 = tf.reduce_mean(tf.reduce_sum(tf.square(y[20] - tf.squeeze(x[20, :, :])), 1))
    ...
    return loss1, loss2, loss3, loss_Linf, loss, loss_comparison, loss_comparison_30
...
def try_net(data_val, params):
    ...
    loss1, loss2, loss3, loss_Linf, loss, loss_comparison, loss_comparison_30 = define_loss(x, y, g_list, weights, biases, params)
    ...
    for f in range(params['data_train_len'] * params['num_passes_per_file']):
        ...
        for step in range(params['num_steps_per_batch'] * num_batches):
            ...
            if step % 20 == 0:
                ...
                train_val_error[count, 18] = sess.run(loss_comparison_30, feed_dict=feed_dict_train_loss)
                train_val_error[count, 19] = sess.run(loss_comparison_30, feed_dict=feed_dict_val)
                print("Loss comparison_1 on val: " + str(train_val_error[count, 17]))
                print("Loss comparison_30 on val: " + str(train_val_error[count, 19]))
